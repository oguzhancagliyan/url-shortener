    ...
    CancellationToken cancellationToken) =>
{
    ...
    var response = await handler.HandleAsync(
        new CreateShortUrlCommand(request.Url, request.ExpiresAtUtc),
...
app.MapGet("/{code}", async Task<Results<RedirectHttpResult, NotFound>> (
    ...
    CancellationToken cancellationToken) =>
{
    var target = await handler.HandleAsync(code, cancellationToken);
    return target is null ? TypedResults.NotFound() : TypedResults.Redirect(target, permanent: false);
});

api.MapGet("/urls/{code}/analytics", async Task<Ok<object>> (
    [FromRoute] string code,
    ...
    CancellationToken cancellationToken) =>
{
    ...
    return TypedResults.Ok(new
    {
        ...
        analytics.LastResolvedAtUtc
    });
});

app.Run();

public sealed record CreateShortUrlRequest(string Url, DateTimeOffset? ExpiresAtUtc);
